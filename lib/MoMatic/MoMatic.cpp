...
void MoMatic::momatic_setup(unsigned int *registri, unsigned int size) {
	...
	momatic_set_id();
 	...
 	MoMatic::modbus_configure(BAUD_RATE, SERIAL_8N2, id, TX_ENABLE_PIN, size, registri);
...
// function definitions for modbus comunication
void exceptionResponse(unsigned char exception);
unsigned int calculateCRC(unsigned char bufferSize);
void sendPacket(unsigned char bufferSize);
void MoMatic::modbus_configure(long baud,
                      ...
                      unsigned int* _regs)
{
	MoMatic::modbus_update_comms(baud, byteFormat, _slaveID);
...
void MoMatic::modbus_update_comms(long baud, unsigned char byteFormat, unsigned char _slaveID)
{
	//(*mySerial).begin(baud, byteFormat);
	(*mySerial).begin(baud);
	slaveID = _slaveID;
...
unsigned int MoMatic::modbus_update()
{

...
byte tempID[4];
byte tempID2[4];
unsigned int accessCounter[] = { 0,0,0,0,0 };
...
void setup()
{
	...
	//modbus_configure(9600,1,4,HOLDING_REGS_SIZE,0);
...
void DumpCard() {
	...
	byte byteLow;
	byte blockAddr = 8;
	byte trailerBlock = 11; //change to trailer block 7 when add new master
	...
	if (status != MFRC522::STATUS_OK) {
		...
		Serial.println(rfid[0].GetStatusCodeName(status));
		digitalWrite(RED_LED, HIGH);
		digitalWrite(GREEN_LED, HIGH);
		delay(3000);
		return;
	...
	while (blockAddr < totalBlocks - 1) {
		...
		if (buffer[0] == 0 &&
			...
			buffer[4] == 0 &&
			buffer[5] == 0) {
			Serial.print("Trovato blocco vuoto: ");
			Serial.print(blockAddr);
			Serial.println();
		}
		else {
			Serial.println("trovato blocco pieno");
			byteHigh = buffer[4];
			byteLow = buffer[5];
